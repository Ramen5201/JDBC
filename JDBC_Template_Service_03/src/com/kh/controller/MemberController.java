...
public class MemberController {
	...
	public void insertMember(String userId, String userPwd, String userName, String gender, String age,
		...
		Member m = new Member(userId, userPwd, userName, gender, 
								Integer.parseInt(age),	email,  phone,  address,  hoppy);	
		
		int result = new MemberService().insertMember(m);
		
		if(result > 0) {
			new MemberMenu().displaySuccess("성공적으로 회원 추가를 완료 하였습니다.");
		} else {
	...
	public void updateMember(String userId,String userPwd,String email,String phone,String address) {
		...
		if(result > 0) {
			...
			new MemberMenu().displayFail("회원정보 변경에 실패하셨습니다.");
		}
	}
	...
	public void deleteMember(String userId) {
		...
		if(result > 0) {
			new MemberMenu().displaySuccess(userId + "회원 삭제 성공");
		} else {
			new MemberMenu().displayFail("삭제 성공");
		}
	}
}
