...
public class RunTest {
	...
	public static void main(String[] args) {
		...
		/*
	       ...
	       * 5) 결과받기 > SELECT문 실행 => ResultSet객체 (조회된 데이터들이 담겨있음) => 6_1) 
	       * 		  > DML문 실행 => int(처리된 행 수)
	       * 
	       * 6_1) ResultSet에 담겨있는 데이터들을 하나씩 하나씩 뽑아서 vo객체에 차근차근 옴겨담기[+ ArrList에 담아주기] 6_2)
	       * 트렌잭션 처리 (성공했다면 commit, 실패했다면 rollback 실행)
	       * 
	       * 7) 다 사용한 JDBC용 객체를 반드시 자원 반납
	       * (close) => 생성된 역순으로
	       * 
	       */

	      /*
	      // 1. 각자 pc(1ocalhost)에 JDBC계정에 연결한 후 TEST테이블에 insert해보기
	      ...
	      try {
	         ...
	         // System.out.println("oracleDeiver 등록성공");

	         // 2) Connection객체 생성 : DB에 연결(ur1, 계정명, 비밀번호)
	         ...
	         // int
	         // 내가 실행할 sql문이 select문일 경우 => stmt.eqcuteQuert("select문") : ResultSet
	         ...
	         // System.out.println("oracleDeiver 등록실패");
	         e.printStackTrace();
	      } catch (SQLException e) {
	         e.printStackTrace();
	      } finally {

	         // 7) 다쓴 JDBC용 객체 자원반납(생선된 역순으로)
	         try {
	            stmt.close();
	            conn.close();
	         } catch (SQLException e) {
		...
		// 실행할 sql문 작성
		String sql = "SELECT * FROM TEST WHERE TNAME = '강호동'";
		...
		try {
			...
			// rset.next() => rest의 다음행이 들어있는지 없는지 확인후 결과반환 + 한행을 내려준다

			while (rset.next()) {
				// 현재 참조하는 rset으로부터 어떤 컬럼에 해당하는 값을 어떤타입으로 뽑을건지 제시해줘야 한다.
				// db에 컬럼명을 제시(대소문자 가리지 않는다.)
